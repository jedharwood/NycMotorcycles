...
export const mailerHandlers = [
  rest.post(`https://api.mailgun.net/v3/${domain}/messages`, async (req, res, ctx) => {
    ...
    if (subject.toLowerCase() === 'fail') {
      return res(ctx.json({ status: 500, id: messageId, message: 'Failed.' }), ctx.delay(3000))
    }

    return res(ctx.json({ status: 200, id: messageId, message: 'Queued. Thank you.' }), ctx.delay(3000))
  })
]
