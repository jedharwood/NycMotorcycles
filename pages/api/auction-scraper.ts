...
const yahooAuctionScraper = async (
    ...
    try {
        const profilePage: Response = await fetch('https://auctions.yahoo.co.jp/seller/lazzamoto?', {
            mode: 'no-cors'
        });
        const html: string = await profilePage.text();
        const dom: JSDOM = new JSDOM(html);
        const document: Document = dom.window.document;
        const auctionListings: NodeListOf<Element> = document.querySelectorAll('.bd.cf');
...
const mapAuctionListing = (listing: Element): ActiveAuction => {
    ...
    const timeRemaining: TimeRemaining = formatRemainingTime(listing.querySelector('dt.rem + dd')?.textContent ?? undefined);

    return {
      title: listing.querySelector('h3')?.textContent ?? undefined,
      url: listing.querySelector('a')?.href ?? undefined,
      imageSrc: listing.querySelector('a > img')?.getAttribute('src') ?? undefined,
      imageAlt: listing.querySelector('a > img')?.getAttribute('alt') ?? undefined,
...
const formatRemainingTime = (timeRemaining: string | undefined): TimeRemaining => {
    ...
    if (timeRemaining.includes(hours)) {
        const time: number = parseInt(timeRemaining.substring(0, timeRemaining.length - 2));
        return { time: time, unit: 'days' }
    } 

    const time: number = parseInt(timeRemaining.substring(0, timeRemaining.length - 1));
    return timeRemaining.includes(days) ? { time: time, unit: 'hours' } : { time: time, unit: 'minutes' };
}
